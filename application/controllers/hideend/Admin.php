...
class Admin extends CI_Controller
{
	...
	{
		$this->template->loadData("activeLink",
			array("admin" => array("menus" => 1)));
		$menus = $this->admin_model->get_menus();
		$this->template->loadContent("hidepage/admin/menus.php", array(
			"menus" => $menus
			)
		);
	}
	...
	public function add_menu()
	{
		$this->template->loadData("activeLink",
			array("admin" => array("menus" => 1)));
		$this->template->loadContent("hidepage/admin/add_menu.php", array(

			)
		);
	}

	public function add_menu_pro()
	{
		$name = $this->common->nohtml($this->input->post("name"));
		$icon = $this->common->nohtml($this->input->post("icon"));
		$dropdown = $this->common->nohtml($this->input->post("dropdown"));

		if(empty($name)) {
			$this->template->error(lang("error_79"));
		}

		$menu_link_count = intval($this->input->post("menu_link_count"));
		$links = array();
		for($i=0;$i<=$menu_link_count;$i++) {
			if(isset($_POST['menu_link_name_' . $i])) {
				$link_name = $this->common->nohtml($this->input->post("menu_link_name_" . $i));
				$link_url = $this->common->nohtml($this->input->post("menu_link_url_" . $i));
				if(!empty($link_name)) {
					$links[] = array("name" => $link_name, "url" => $link_url);
				}
			}
		}

		// Add menu
		$menuid = $this->admin_model->add_menu(array(
			"name" => $name,
			"icon" => $icon,
			"dropdown" => $dropdown
			)
		...
		foreach($links as $link) {
			$this->admin_model->add_menu_link(array(
				"menuid" => $menuid,
				"name" => $link['name'],
				"url" => $link['url']
				)
		...
		redirect(site_url("hideend/admin/menus"));
	}

	public function edit_menu($id)
	{
		$id = intval($id);
		$menu = $this->admin_model->get_menu($id);
		if($menu->num_rows() == 0) {
			$this->template->error(lang("error_80"));
		}
		$menu = $menu->row();

		$links = $this->admin_model->get_menu_links($id);
		$this->template->loadData("activeLink",
			array("admin" => array("menus" => 1)));


		$this->template->loadContent("hidepage/admin/edit_menu.php", array(
			"menu" => $menu,
			"links" => $links
			)
		);
	...
	public function edit_menu_pro($id)
	{
		$id = intval($id);
		$menu = $this->admin_model->get_menu($id);
		if($menu->num_rows() == 0) {
		...
		// Add menu
		$this->admin_model->update_menu($id, array(
			"name" => $name,
			"icon" => $icon,
			"dropdown" => $dropdown
			)
		...
		$this->admin_model->delete_menu_links($id);
	...
	public function delete_menu($id, $hash)
	{
	...
	public function custom_fields()
	{
		...
		$this->template->loadContent("hidepage/admin/custom_fields.php", array(
			"fields" => $fields
			)
		);
	...
	public function add_custom_field_pro()
	{
		...
		redirect(site_url("hideend/admin/custom_fields"));
	}
	...
	public function edit_custom_field($id)
	{
		...
		$field = $field->row();
		$this->template->loadContent("hidepage/admin/edit_custom_field.php", array(
			"field" => $field
			)
		);
	}
	...
	public function premium_users($page=0)
	{
		...
		$this->template->loadContent("hidepage/admin/premium_users.php", array(
			)
		);
	}
	...
	public function premium_users_page()
	{
		...
		foreach($users->result() as $r) {
			...
			$this->datatables->data[] = array(
				...
				'<a href="' . site_url("hideend/edit_member/" . $r->ID) .'" class="btn btn-warning btn-xs" title="'. lang("ctn_55").'"><span class="glyphicon glyphicon-cog"></span></a> <a href="' . site_url("hideend/delete_member/" . $r->ID . "/" . $this->security->get_csrf_hash()) .'" onclick="return confirm(\'' . lang("ctn_86") . '\')" class="btn btn-danger btn-xs" title="'. lang("ctn_57") .'"><span class="glyphicon glyphicon-trash"></span></a>'
			);
		}
	...
	public function user_roles()
	{
		...
		$this->template->loadContent("hidepage/admin/user_roles.php", array(
			"roles" => $roles
			)
		);
	}
	...
	public function add_user_role_pro()
	{
		...
		$content_manager = intval($this->input->post("content_manager"));
		$content_worker = intval($this->input->post("content_worker"));
		...
		redirect(site_url("hideend/admin/user_roles"));
	}
	...
	public function edit_user_role($id)
	{
		...
		$this->template->loadContent("hidepage/admin/edit_user_role.php", array(
			"role" => $role->row()
			)
		);
	}
	...
	public function payment_logs($page = 0)
	{
			   ...
			   var st = $("#search_type").val();
				var table = $("#content-table").DataTable({
				  ...
				  "processing": false,
					...
					"ajax": {
						url : "'.site_url("hideend/admin/payment_logs_page").'",
						type : "GET",
						data : function ( d ) {
		...
		$this->template->loadContent("hidepage/admin/payment_logs.php", array(
			)
		);
	}
	...
	public function payment_plans()
	{
		...
		$this->template->loadContent("hidepage/admin/payment_plans.php", array(
			"plans" => $plans
			)
		);
	}
	...
	public function add_payment_plan()
	{
		...
		redirect(site_url("hideend/admin/payment_plans"));
	}
	...
	public function edit_payment_plan($id)
	{
		...
		$this->template->loadContent("hidepage/admin/edit_payment_plan.php", array(
			"plan" => $plan->row()
			)
		);
	}
	...
	public function payment_settings()
	{
		...
		$this->template->loadContent("hidepage/admin/payment_settings.php", array(
			)
		);
	}
	...
	public function payment_settings_pro()
	{
		...
		redirect(site_url("hideend/admin/payment_settings"));
	...
	public function email_members()
	{
		...
		$this->template->loadContent("hidepage/admin/email_members.php", array(
			"groups" => $groups
			)
		);
	}
	...
	public function email_members_pro()
	{
		...
		redirect(site_url("hideend/admin/email_members"));
	}
	...
	public function user_groups()
	{
		...
		$groups = $this->admin_model->get_user_groups();
		$this->template->loadContent("hidepage/admin/groups.php", array(
			"groups" => $groups
			)
		);
	}
	...
	public function add_group_pro()
	{
		...
		redirect(site_url("hideend/admin/user_groups"));
	}
	...
	public function edit_group($id)
	{
		...
		$this->template->loadContent("hidepage/admin/edit_group.php", array(
			"group" => $group->row()
			)
		);
	}
	...
	public function view_group($id, $page=0)
	{
		...
		$config['base_url'] = site_url("hideend/view_group/" . $id);
		...
		$this->template->loadContent("hidepage/admin/view_group.php", array(
			"group" => $group->row(),
			"users" => $users,
	...
	public function add_user_to_group_pro($id)
	{
		...
		redirect(site_url("hideend/admin/view_group/" . $id));
	}
	...
	public function email_templates()
	{
		...
		$this->template->loadContent("hidepage/admin/email_templates.php", array(
	...
	public function edit_email_template($id)
	{
		...
		$this->template->loadContent("hidepage/admin/edit_email_template.php", array(
	...
	public function edit_email_template_pro($id)
	{
		...
		redirect(site_url("hideend/admin/email_templates"));
	}
	...
	public function ipblock()
	{
		...
		$this->template->loadContent("hidepage/admin/ipblock.php", array(
			"ipblock" => $ipblock
			)
		);
	}
	...
	public function add_ipblock()
	{
		...
		redirect(site_url("hideend/admin/ipblock"));
	}
	...
	public function members()
	{
		...
		$this->template->loadContent("hidepage/admin/members.php", array(
	...
	public function members_page()
	{
		...
		foreach($members->result() as $r) {
			...
			$this->datatables->data[] = array(
				...
				date($this->settings->info->date_format, $r->joined),
				$provider,
				'<a href="'.site_url("hideend/edit_member/" . $r->ID).'" class="btn btn-warning btn-xs" title="'.lang("ctn_55").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-cog"></span></a> <a href="'.site_url("hideend/delete_member/" . $r->ID . "/" . $this->security->get_csrf_hash()).'" class="btn btn-danger btn-xs" onclick="return confirm(\''.lang("ctn_317").'\')" title="'.lang("ctn_57").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-trash"></span></a>'
			);
		}
		echo json_encode($this->datatables->process());
	}
	...
	public function search_member()
	{
		...
		$this->template->loadContent("hidepage/admin/search_members.php", array(
			"members" => $members,
			)
		);
	}
	...
	public function edit_member($id)
	{
		...
		$this->template->loadContent("hidepage/admin/edit_member.php", array(
			"member" => $member->row(),
	...
	public function edit_member_pro($id)
	{
		...
		redirect(site_url("hideend/members"));
	}
	...
	public function social_settings()
	{
		...
		$this->template->loadContent("hidepage/admin/social_settings.php", array(
			)
		);
	}
	...
	public function social_settings_pro()
	{
		...
		redirect(site_url("hideend/admin/social_settings"));
	}
	...
	public function settings()
	{
		...
		$roles = $this->admin_model->get_user_roles();
		$currencyRate = $this->admin_model->getRatioCurrency("USD","IDR");
		$this->template->loadContent("hidepage/admin/settings.php", array(
			"roles" => $roles,
	...
	public function settings_pro()
	{
		...
		$currencyRate = $this->common->nohtml($this->input->post("currencyRate"));
		...
		$image_fr_num = intval($this->input->post("image_num"));
		$upload_path_rel =
		...
		$menu_highlight = intval($this->input->post("menu_highlight"));
		...
		if ($_FILES['userfile']['size'] > 0) {
			$this->upload->initialize(array(
		       "upload_path" => $this->settings->info->upload_path."/setting/",
		       "overwrite" => FALSE,
		       "max_filename" => 300,
		       "encrypt_name" => TRUE,
		       "remove_spaces" => TRUE,
		    ...
		    $data = $this->upload->data();
			$this->common->resizecropImageCI($data,271,40);
		    $image = $data['file_name'];
			$namafile=explode(".",$data['file_name']);
		    $image = "/setting/".$namafile[0];
		} else {
		...
		if ($_FILES['userfilesiteothers']['size'] > 0) {
			$this->upload->initialize(array(
		       "upload_path" => $this->settings->info->upload_path."/setting/",
		       "overwrite" => FALSE,
		       "max_filename" => 300,
		    ...
		    if (!$this->upload->do_upload('userfilesiteothers')) {
		    	$this->template->error(lang("error_21")
		    	.$this->upload->display_errors());
		    }

		    $data = $this->upload->data();
			$this->common->resizecropImageCI($data,262,38);
		    $image_other = $data['file_name'];
			$namafile=explode(".",$data['file_name']);
		    $image_other = "/setting/".$namafile[0];
		} else {
			$image_other = $this->settings->info->site_logo_other;
		}		
		
		$this->load->library("upload");

		if ($_FILES['userfilefavico']['size'] > 0) {
			$this->upload->initialize(array(
		       "upload_path" => $this->settings->info->upload_path."/setting/",
		       "overwrite" => FALSE,
		       "max_filename" => 300,
		    ...
		    if (!$this->upload->do_upload('userfilefavico')) {
		    	$this->template->error(lang("error_21")
		    	.$this->upload->display_errors());
		    }

		    $data = $this->upload->data();
			$this->common->resizecropImageCI($data,67,67);
		    $image_favicon = $data['file_name'];
			$namafile=explode(".",$data['file_name']);
		    $image_favicon = "/setting/".$namafile[0];
		} else {
			$image_favicon = $this->settings->info->favicon;
		}
		$ratio=$this->admin_model->getRatioCurrency("USD","IDR");
		if ($currencyRate != $ratio->rates ){
			$this->admin_model->setCurrency(array(
					"from" => "USD",
					"to" => "IDR",
					"rates" => $currencyRate,
				)
		...
		$this->admin_model->updateSettings(
			array(
				...
				"site_logo"=> $image,
				"site_logo_other"=> $image_other,
				"favicon"=> $image_favicon,			
				"ext_site_logo"=> isset($namafile[1])?$namafile[1]:"jpg",
				"ext_site_logo_other"=> isset($namafile[1])?$namafile[1]:"jpg",
				"ext_favicon"=> isset($namafile[1])?$namafile[1]:"jpg",
		...
		$this->session->set_flashdata("globalmsg", lang("success_13"));
		redirect(site_url("hideend/admin/settings"));
	}
