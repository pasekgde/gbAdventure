...
class User_Model extends CI_Model 
{
	...
	{
		return $this->db->select("count(*) num, joined")
				->group_by('joined')	
				->get("users");
	}
	...
	public function search_by_amount($amount) 
	{
		return $this->db->select("users.ID, users.username, users.email, users.first_name, 
			users.last_name, users.joined, users.amount,users.paymentstatus,")
		->where("paymentstatus",0)
		->like("users.amount", $amount,'both')
		->get("users");
