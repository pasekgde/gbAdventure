...
class SendtochannelCommand extends AdminCommand
{
            ...
            case 4:
                if (!$text_yes_or_no || $notes['last_message_id'] === $message->getMessageId()) {
                    ...
                    if ($notes['message_type'] !== 'command') {
                        ...
                        $isPreview = true;
                        $this->sendBack(new Message($notes['message'], $this->telegram->getBotUsername()), $data, $isPreview);
     ...
     * @throws \Longman\TelegramBot\Exception\TelegramException
     */
    protected function sendBack(Message $message, array $data, $isPreview = false)
    {
        $type = $message->getType();
        in_array($type, ['command', 'text'], true) && $type = 'message';
       ...
       if (!$ispreview) { 
            $inline_keyboard = new InlineKeyboard([
                        ...
                        ['text' => 'test', 'callback_data' => 'identifier'],
                    ]
                    );
