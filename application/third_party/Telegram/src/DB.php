...
class DB
{
    ...
    public static function selectTelegramUpdate($limit = null, $id = null)
    {
        ...
        try {
            $sql = 'SELECT `id` FROM `' . TB_TELEGRAM_UPDATE_T . '`';
    ...
    public static function insertTelegramUpdate(
        ...
        try {
            $sth = self::$pdo->prepare('
                INSERT IGNORE INTO `' . TB_TELEGRAM_UPDATE_T . '`
    ...
    public static function insertUser(User $user, $date, Chat $chat = null)
    {
        ...
        if ($chat instanceof Chat) {
            ...
            try {
                $sth = self::$pdo->prepare('
                    INSERT IGNORE INTO `' . TB_USER_CHAT_T . '`
                    (`user_id`, `chat_id`)
                    VALUES
                    (:user_id, :chat_id)
                ');
    ...
    public static function insertInlineQueryRequest(InlineQuery $inline_query)
    {
        ...
        try {
            $sth = self::$pdo->prepare('
                INSERT IGNORE INTO `' . TB_INLINE_QUERY_T . '`
    ...
    public static function insertChosenInlineResultRequest(ChosenInlineResult $chosen_inline_result)
    {
        ...
        try {
            $sth = self::$pdo->prepare('
                INSERT INTO `' . TB_CHOSEN_INLINE_RESULT_T . '`
    ...
    public static function insertCallbackQueryRequest(CallbackQuery $callback_query)
    {
        ...
        try {
            $sth = self::$pdo->prepare('
                INSERT IGNORE INTO `' . TB_CALLBACK_QUERY_T . '`
    ...
    public static function insertMessageRequest(Message $message)
    {
        ...
        try {
            $sth = self::$pdo->prepare('
                INSERT IGNORE INTO `' . TB_MESSAGE_T . '`
                (
    ...
    public static function insertEditedMessageRequest(Message $edited_message)
    {
        ...
        try {
            $sth = self::$pdo->prepare('
                INSERT IGNORE INTO `' . TB_EDITED_MESSAGE_T . '`
    ...
    public static function selectChats(
        ...
        try {
            $query = '
                SELECT * ,
                ' . TB_CHAT_T . '.`id` AS `chat_id`,
                ' . TB_CHAT_T . '.`created_at` AS `chat_created_at`,
                ' . TB_CHAT_T . '.`updated_at` AS `chat_updated_at`
            ';
            if ($select_users) {
                $query .= '
                    , ' . TB_USER_T . '.`id` AS `user_id`
                    FROM `' . TB_CHAT_T . '`
                    LEFT JOIN `' . TB_USER_T . '`
                    ON ' . TB_CHAT_T . '.`id`=' . TB_USER_T . '.`id`
                ';
            } else {
                $query .= 'FROM `' . TB_CHAT_T . '`';
            }
            ...
            if (!$select_groups || !$select_users || !$select_super_groups) {
                ...
                $select_groups && $chat_or_user[] = TB_CHAT_T . '.`type` = "group"';
                $select_super_groups && $chat_or_user[] = TB_CHAT_T . '.`type` = "supergroup"';
                $select_users && $chat_or_user[] = TB_CHAT_T . '.`type` = "private"';

                $where[] = '(' . implode(' OR ', $chat_or_user) . ')';
            }

            if (null !== $date_from) {
                $where[]              = TB_CHAT_T . '.`updated_at` >= :date_from';
                $tokens[':date_from'] = $date_from;
            }

            if (null !== $date_to) {
                $where[]            = TB_CHAT_T . '.`updated_at` <= :date_to';
                $tokens[':date_to'] = $date_to;
            }

            if (null !== $chat_id) {
                $where[]            = TB_CHAT_T . '.`id` = :chat_id';
            ...
            if (null !== $text) {
                if ($select_users) {
                    $where[] = '(
                        LOWER(' . TB_CHAT_T . '.`title`) LIKE :text
                        OR LOWER(' . TB_USER_T . '.`first_name`) LIKE :text
                        OR LOWER(' . TB_USER_T . '.`last_name`) LIKE :text
                        OR LOWER(' . TB_USER_T . '.`username`) LIKE :text
                    )';
                } else {
                    $where[] = 'LOWER(' . TB_CHAT_T . '.`title`) LIKE :text';
                }
            ...
            $query .= ' ORDER BY ' . TB_CHAT_T . '.`updated_at` ASC';
    ...
    public static function getTelegramRequestCount($chat_id = null, $inline_message_id = null)
    {
        ...
        try {
            $sth = self::$pdo->prepare('SELECT 
                (SELECT COUNT(*) FROM `' . TB_REQUEST_LIMITER_T . '` WHERE `created_at` >= :date) as LIMIT_PER_SEC_ALL,
                (SELECT COUNT(*) FROM `' . TB_REQUEST_LIMITER_T . '` WHERE ((`chat_id` = :chat_id AND `inline_message_id` IS NULL) OR (`inline_message_id` = :inline_message_id AND `chat_id` IS NULL)) AND `created_at` >= :date) as LIMIT_PER_SEC,
                (SELECT COUNT(*) FROM `' . TB_REQUEST_LIMITER_T . '` WHERE `chat_id` = :chat_id AND `created_at` >= :date_minute) as LIMIT_PER_MINUTE
            ');
    ...
    public static function insertTelegramRequest($method, $data)
    {
        ...
        try {
            $sth = self::$pdo->prepare('INSERT INTO `' . TB_REQUEST_LIMITER_T . '`
                (
